#!/usr/bin/env zsh
# check-docs.zsh — validate documentation cross-links for a Quarto-published repo.
#
# Run from the repo root (or pass the root as $1). The file list is derived,
# not configured: every tracked markdown file, plus anything on the
# _quarto.yml render list (covers new not-yet-tracked files like index.md).
#
# Two passes:
#   1. Every relative markdown link/image resolves to an existing file, and
#      its #anchor (if any) to a real heading. Headings must be ATX (`##`) —
#      setext (underlined) headings are not collected as anchors.
#   2. Site reachability: every relative .md link in a RENDERED file must
#      target another rendered file — a target that exists in the repo but is
#      missing from the Quarto render list 404s on the published site (Quarto
#      only rewrites links to rendered inputs). Mirrors Quarto's rules:
#      explicit entries, glob entries (which skip README.md files), and "!"
#      exclusions.
#
...
if [[ ! -f _quarto.yml ]]; then
    print -r -- "check-docs: no _quarto.yml in $root — run from the repo root"
    exit 1
fi
...
# ── Parse the _quarto.yml render list ─────────────────────────────────────
# Produces $expanded (rendered files) following Quarto's rules: explicit
# entries, glob entries (which skip README.md files), and "!" exclusions.
local f l t entry rf
...
# ── Build the file list: tracked markdown ∪ render list ──────────────────
local -a files
files=( ${(f)"$(git ls-files -- '*.md' 2>/dev/null)"} )
for rf in $expanded; do
    [[ $rf == *.md ]] || continue
    (( ${files[(Ie)$rf]} )) || files+=("$rf")
done
if (( ! ${#files} )); then
    print -r -- "check-docs: no markdown files found"
    exit 1
fi
...
if (( errors )); then
    ...
    print -r -- "check-docs: OK (${#files} files checked, ${#expanded} rendered)"
fi
exit $errors
